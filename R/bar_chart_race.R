...
bar_chart_race <- function(df, cat_col, val_col, time_col,
  ...
  df <- fill_missing_times(df, {{cat_col}}, {{val_col}}, {{time_col}}) 
  
  df <- filter_top_ranks(df, {{val_col}}, {{time_col}}, max_bars)
  
  # interpolate between finer time values for smoother transitions
  df <- tween_more_times(df, {{cat_col}}, {{val_col}}, {{time_col}})
...
fill_missing_times <- function(df, cat_col, val_col, time_col) {
  ...
  df %>%
    ...
    tidyr::complete({{time_col}} := tidyr::full_seq({{time_col}}, 1)) %>%
